...
class sphere_feature_extraction(nn.Module):
  def __init__(self,num_ch_out, in_height, in_width, sphereType):
    ...
    self.lastconv = nn.Sequential(convbn(256, 128, 1, 1, 0, 1), nn.ReLU(inplace=True), convbn(128, 128, 3, 1, 1, 1), nn.ReLU(inplace=True), convbn(128, num_ch_out, 1, 1, 0, 1), nn.ReLU(inplace=True))
