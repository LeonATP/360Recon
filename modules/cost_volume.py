...
class FeatureVolumeManager(CostVolumeManager):
    ...
    def __init__(self, 
        ...
        #!For equirectangular images,the mask will always be true,so we abandon it!
        #num_mask_channels = 1+num_source_views
        num_num_dot_channels = num_source_views        
        num_pose_penalty_channels = 3 * (num_source_views)

        # update mlp channelss
    ...
    def build_cost_volume(self, 
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            """
            # Calculate ray directions for current frame while keeping original variables unchanged
            cur_points_rays_B3hw = F.normalize(
                world_points_B4N[:, :3, :], 
                dim=1
            ).view(
            ...
            cur_points_rays_bk3hw = tensor_B_to_bM(
                cur_points_rays_B3hw, 
                batch_size=batch_size, 
                num_views=num_src_frames
            )

            # Calculate ray directions for source frames while keeping original variables unchanged
            src_poses_B44 = tensor_bM_to_B(src_poses)
            src_points_rays_B3hw = get_camera_rays(
                src_poses_B44,
                world_points_B4N[:, :3, :],
            ...
            """
            # Calculate latitude angle information for current frame and store in new variable
            normalized_cur_rays = F.normalize(
                world_points_B4N[:, :3, :], 
                dim=1
            )

            cur_latitude = torch.atan2(
                -normalized_cur_rays[:, 1, :],  # Negate y component to make latitude positive upward
                torch.sqrt(normalized_cur_rays[:, 0, :] ** 2 + normalized_cur_rays[:, 2, :] ** 2)
            ).unsqueeze(1)  # Add channel dimension to make shape (B, 1, H, W)

            cur_latitude_bk1hw = tensor_B_to_bM(
                cur_latitude.view(
                    -1,
                    1,
                    self.matching_height, 
            ...
            # Calculate latitude angle information for source frames and store in new variable
            normalized_src_rays = F.normalize(
                get_camera_rays(
                    src_poses_B44,
                    world_points_B4N[:, :3, :],
                    in_camera_frame=False
            ...
            src_latitude = torch.atan2(
                -normalized_src_rays[:, 1, :],  # Negate y component to make latitude positive upward
                torch.sqrt(normalized_src_rays[:, 0, :] ** 2 + normalized_src_rays[:, 2, :] ** 2)
            ).unsqueeze(1)  # Add channel dimension to make shape (B, 1, H, W)

            src_latitude_bk1hw = tensor_B_to_bM(
                src_latitude.view(
                    -1, 
                    1, 
                    self.matching_height, 
            ...
            # Combine latitude angle information from current frame and source frames
            mask = combine_dims(
                        torch.cat(
                                [cur_latitude_bk1hw[:,0,:,:,:][:,None,:,:,:], 
                                    src_latitude_bk1hw],
                                dim=1,
                        ), 
                        1, 
            ...
            # run through the MLP!
            mlp_input_features_bhwc = mlp_input_features_bchw.permute(0,2,3,1).double()
