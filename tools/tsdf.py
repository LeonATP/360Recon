...
class TSDF:
    ...
    def __init__(
        ...
        self.color_values = color_values.half()
    ...
    def from_file(cls, tsdf_file):
        ...
        color_values = torch.zeros(*tsdf_values.shape,3,dtype=torch.float32)
        voxel_coords = cls.generate_voxel_coords(origin, tsdf_values.shape[1:], voxel_size)

        return TSDF(voxel_coords, tsdf_values, tsdf_weights, voxel_size, color_values)
    ...
    def from_bounds(cls, bounds: dict, voxel_size: float):
        ...
        return TSDF(voxel_coords, tsdf_values, tsdf_weights, voxel_size, origin, color_values)
    ...
    def cuda(self):
        ...
        self.color_values = self.color_values.cuda()
    ...
    def cpu(self):
        ...
        self.color_values = self.color_values.cpu()
        if self.tsdf_weights is not None:
    ...
    def to_mesh(self, scale_to_world=True, export_single_mesh=False):
        ...
        color_values_np = self.color_values.detach().cpu().numpy()
        voxel_indices = np.round(verts).astype(int)
        colors = color_values_np[voxel_indices[:, 0], voxel_indices[:, 1], voxel_indices[:, 2]]
        colors = (colors * 255).astype(np.uint8).copy(order='C')
        ...
        mesh = trimesh.Trimesh(vertices=verts, faces=faces, vertex_colors=colors, normals=norms)
        return mesh
        ...
        if save_mesh:
            ...
            trimesh.exchange.export.export_mesh(
                mesh, os.path.join(savepath, 
                                    filename).replace(".bin", ".ply"), "ply",vertex_colors=True)
...
class TSDFFuser:
    ...
    def project_to_camera(self, cam_T_world_T_b44, K_b44):
        ...
        world_points_b4N = \
            self.hom_voxel_coords_14hwd.expand(batch_size, 4, *self.shape).flatten(start_dim=2)
        cam_points_b4N = cam_T_world_T_b44@world_points_b4N
        depth_b1N = torch.sqrt((cam_points_b4N[:,0,:])**2+(cam_points_b4N[:,2,:])**2)
        depth=torch.sqrt((cam_points_b4N[:,0,:])**2+(cam_points_b4N[:,1,:])**2+(cam_points_b4N[:,2,:])**2)
        spherical_points_b4N = cam_points_b4N
        spherical_points_b4N[:,0,:]=torch.atan2(cam_points_b4N[:,0,:],cam_points_b4N[:,2,:])
        spherical_points_b4N[:,1,:]=torch.atan((-cam_points_b4N[:,1,:])/depth_b1N)
        spherical_points_b4N[:,2,:]=1
        cam_points_b3N = K_b44[:, :3] @ cam_points_b4N
        cam_points_b3N[:,2,:]=depth
        
        return cam_points_b3N

    def integrate_depth_color(
                self,
                depth_b1hw,
                color_b3hw,
                cam_T_world_T_b44,
        ...
        if self.use_gpu:
            ...
            color_b3hw = color_b3hw.cuda().half()
        ...
        # Sample the color using grid sample
        sampled_color_b3hw = TF.grid_sample(input=color_b3hw,
                                            grid=pixel_coords_bhw2,
                                            mode="nearest",
                                            padding_mode="zeros",
                                            align_corners=False)
        sampled_color_b3N = sampled_color_b3hw.flatten(start_dim=2)
        ...
        for tsdf_val_1N, valid_points_1N, confidence_1N, sampled_color_3N in zip(tsdf_vals_b1N,
                                                               valid_points_b1N,
                                                               confidence_b1N,
                                                               sampled_color_b3N):
            ...
            old_color_vals = self.color_values[valid_points_hwd]
            new_color_vals = sampled_color_3N[:,valid_points_1N.squeeze(0)]
            new_color_vals = new_color_vals.transpose(0,1)
            old_weights = old_weights.unsqueeze(1)
            new_weights = new_weights.unsqueeze(1)
            total_weights = total_weights.unsqueeze(1)
            result = (old_color_vals * old_weights + new_color_vals * new_weights) / total_weights
            self.color_values[valid_points_hwd,:] = result