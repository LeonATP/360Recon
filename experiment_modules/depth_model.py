...
from modules.networks import (CVEncoder, DepthDecoderPP, UNetMatchingEncoder,
                             ResnetMatchingEncoder,ResnetMatchingEncoder_ERP,Encoder)
...
def visualize_and_save_cost_volume(cost_volume, output_dir="output", colormap='viridis', normalize=True):
    # Select cost volume of the 0th image, shape is (64, 128, 256)
    cost_volume_0 = cost_volume[0]  # Take the 0th image from [2, 64, 128, 256], get (64, 128, 256)
    
    # Create directory if output directory does not exist
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # Visualize each depth dimension (64 dimensions)
    for i in range(cost_volume_0.shape[0]):  # Iterate through 64 depth dimensions
        # Get cost values for the i-th depth dimension
        cost_map = cost_volume_0[i]  # Shape is (128, 256)
        
                # Convert PyTorch tensor to numpy array if cost_map is a tensor
        if isinstance(cost_map, torch.Tensor):
            cost_map = cost_map.cpu().numpy()
        
        # Normalize (optional)
        if normalize:
            cost_map = exposure.rescale_intensity(cost_map, in_range='image')  # Use skimage normalization method

        # Generate heatmap or other types of visualization using matplotlib
        plt.imshow(cost_map, cmap=colormap, interpolation='nearest')
        plt.colorbar()  # Add colorbar

        # Save as image file
        output_path = os.path.join(output_dir, f"cost_map_{i:03d}.png")
        plt.savefig(output_path)
        plt.close()  # Close current figure to avoid memory usage
...
class DepthModel(pl.LightningModule):
    ...
    def __init__(self, opts):
        ...
        if "efficientnet" in self.run_opts.image_encoder_name:
            ...
            #self.encoder.num_ch_enc = self.encoder.num_ch_enc
        else:
        ...
        elif "resnet_ERP" == self.run_opts.matching_encoder_type:
            self.matching_model = ResnetMatchingEncoder_ERP(18, 
                                            self.run_opts.matching_feature_dims)
            #self.matching_model = sphere_feature_extraction(self.run_opts.matching_feature_dims,512,1024,'ERP')
        else:
            raise ValueError(f"Unrecognized option {self.run_opts.matching_encoder_type} "
                            f"for matching encoder type!")
        #self.replace_conv_with_sphereconv(self.depth_decoder)
        #self.replace_conv_with_sphereconv(self.encoder)
        #self.replace_conv_with_sphereconv(self.cost_volume_net)
        #self.replace_conv_with_sphereconv(self.matching_model)
    ...
    def forward(
        ...
        # Visualize and save heatmaps for 64 depth dimensions of the 0th image
        #visualize_and_save_cost_volume(cost_volume,colormap='jet', normalize=True)
        if flip:
    ...
    def step(self, phase, batch, batch_idx):
        ...
        with torch.inference_mode():
            ...
            if (is_train and 
                ...
                for i in range(self.run_opts.batch_size):
