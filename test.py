        ...
        for line in lines:
            ...
            if scene_name == scan:
                try:
                    ...
                    # If a field cannot be converted to float, skip this line
                    continue
        
        # If no matching scene is found, return default values
        return     {
                        "xmin": -10,
                        "xmax": 10,
                        "ymin": -10,
                        ...
                        "zmin": -5,
                        "zmax": 5
                    } # Default return, use standard bounds
...
def main(opts):
    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            bounds = generate_bounds_for_scan_from_file(scan,'data_splits/Matterport3d/matterport3d_camera_center.txt')  # Custom function to generate scan-specific bounds
            ...
            for batch_ind, batch in enumerate(tqdm(dataloader)):
                ...
                start_time.record()
                # Record GPU memory usage before each iteration
                #before_allocated = torch.cuda.memory_allocated(device)  # Allocated memory (bytes)
                #before_cached = torch.cuda.memory_reserved(device)  # Reserved memory (bytes)
                ...
                #valid_mask_b[:, :80, :] =False  # Top 320 rows
                #valid_mask_b[:, -80:, :] = False  # Bottom 320 rows
