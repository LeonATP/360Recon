...
class MSGradientLoss(nn.Module):
    ...
    def forward(self, depth_gt: Tensor, depth_pred: Tensor) -> Tensor:
        ...
        for depth_pred_down, depth_gtn_down in zip(depth_pred_pyr, depth_gtn_pyr):
            ...
            """
            if not grad_error.isfinite().all():
                raise ValueError("Non-finite values in gradient error!")
            """
        return grad_loss
...
class MVDepthLoss(nn.Module):
    ...
    def get_valid_mask(
        ...
        #valid_mask_b1hw =torch.logical_and(valid_mask_b1hw,valid_mask_polar_b1hw)
