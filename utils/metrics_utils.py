...
def compute_depth_metrics_batched(gt_bN, pred_bN, valid_masks_bN, mult_a=False):
    ...
    #print("Minimum gt_bN:", torch.min(gt_bN[valid_masks_bN]))
    #print("Minimum pred_bN:", torch.min(pred_bN[valid_masks_bN]))
    
    # 检查 gt_bN 和 pred_bN 中是否有非正值
    if torch.any(gt_bN <= 0):
        raise ValueError("Ground truth (gt_bN) contains non-positive values.")
    if torch.any(pred_bN <= 0):
        raise ValueError("Predictions (pred_bN) contain non-positive values.")
    
    # 进一步检查 NaN 或 Inf
    if torch.isinf(gt_bN).any() or torch.isinf(pred_bN).any():
        raise ValueError("gt_bN or pred_bN contains Inf values after masking.")
    ...
    rmse_log_bN = (torch.log10(gt_bN) - torch.log10(pred_bN)) ** 2
    # 检查 rmse_log_bN 中是否存在 inf 或 NaN
    if torch.isinf(rmse_log_bN).any():
        raise ValueError("rmse_log_bN contains inf or NaN values. Check gt_bN and pred_bN for non-positive values.")
...
def compute_depth_metrics_batched(gt_bN, pred_bN, valid_masks_bN, mult_a=False):
    ...
    gt_bN[~valid_masks_bN] = 0.1
    pred_bN[~valid_masks_bN] = 0.1
    gt_bN[gt_bN<=0.1] = 0.1
    pred_bN[pred_bN<=0.1] = 0.1
    gt_bN[gt_bN >= 16] = 16
    pred_bN[pred_bN >= 16] = 16
    ...
    a_val = (thresh_bN < (1.0+0.05)     ).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a5"] = torch.mean(a_val, dim=1)

    a_val = (thresh_bN < (1.0+0.10)     ).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a10"] = torch.mean(a_val, dim=1) 

    a_val = (thresh_bN < (1.0+0.25)     ).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a25"] = torch.mean(a_val, dim=1)

    a_val = (thresh_bN < (1.0+0.10)     ).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a0"] = torch.mean(a_val, dim=1)

    a_val = (thresh_bN < (1.0+0.25)     ).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a1"] = torch.mean(a_val, dim=1)

    a_val = (thresh_bN < (1.0+0.25) ** 2).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a2"] = torch.mean(a_val, dim=1)

    a_val = (thresh_bN < (1.0+0.25) ** 3).float()
    #a_val[~valid_masks_bN] = torch.nan
    a_dict[f"a3"] = torch.mean(a_val, dim=1)
    ...
    rmse_bN = (gt_bN - pred_bN) ** 2
    rmse_b = torch.sqrt(torch.mean(rmse_bN, dim=1))

    rmse_log_bN = (torch.log10(gt_bN) - torch.log10(pred_bN)) ** 2
    # 检查 rmse_log_bN 中是否存在 inf 或 NaN
    if torch.isinf(rmse_log_bN).any():
        raise ValueError("rmse_log_bN contains inf or NaN values. Check gt_bN and pred_bN for non-positive values.")
    rmse_log_b = torch.sqrt(torch.mean(rmse_log_bN, dim=1))

    abs_rel_b = torch.mean(torch.abs(gt_bN - pred_bN) / gt_bN, dim=1)

    sq_rel_b = torch.mean((gt_bN - pred_bN) ** 2 / gt_bN, dim=1)

    abs_diff_b = torch.mean(torch.abs(gt_bN - pred_bN), dim=1)
