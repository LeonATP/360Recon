...
class BackprojectDepth(jit.ScriptModule):
    ...
    def forward(self, depth_b1hw: Tensor, invK_b44: Tensor) -> Tensor:
        ...
        """
        #!Use equirectaangular camera model(Spherical coordinates)
        spherical_cam_points_b3N = torch.matmul(invK_b44[:, :3, :3], self.pix_coords_13N)
        cam_points_b3N=torch.ones(spherical_cam_points_b3N.shape).to(spherical_cam_points_b3N.device)
        cam_points_b3N[:,0,:]=depth_b1hw.flatten(start_dim=1) *torch.cos(spherical_cam_points_b3N[:,1,:])*torch.sin(spherical_cam_points_b3N[:,0,:])
        cam_points_b3N[:,1,:]=depth_b1hw.flatten(start_dim=1)*(-torch.sin(spherical_cam_points_b3N[:,1,:]))
        cam_points_b3N[:,2,:]=depth_b1hw.flatten(start_dim=1) *torch.cos(spherical_cam_points_b3N[:,0,:])*torch.cos(spherical_cam_points_b3N[:,1,:])
        cam_points_b4N = to_homogeneous(cam_points_b3N, dim=1)
        return cam_points_b4N.double()
...
class Project3D(jit.ScriptModule):
    ...
    def forward(self, points_b4N: Tensor,
        ...
        """
        #!Use equirectaangular camera model(Spherical coordinates)
        points_b4N=points_b4N.to(cam_T_world_b44.device).double()
        cam_T_world_b44 = cam_T_world_b44.double()
        cam_points_b4N = cam_T_world_b44 @ points_b4N
        plane_depth_b1N = torch.sqrt(torch.square(cam_points_b4N[:,0,:])+torch.square(cam_points_b4N[:,2,:]))
        distance_b1N=torch.sqrt(torch.square(cam_points_b4N[:,0,:])+torch.square(cam_points_b4N[:,1,:])+torch.square(cam_points_b4N[:,2,:]))
        spherical_points_b4N = cam_points_b4N
        spherical_points_b4N=torch.ones(cam_points_b4N.shape).to(cam_points_b4N.device)
        spherical_points_b4N[:,0,:]=torch.atan2(cam_points_b4N[:,0,:],cam_points_b4N[:,2,:])
        spherical_points_b4N[:,1,:]=torch.atan((-cam_points_b4N[:,1,:])/plane_depth_b1N)
        spherical_points_b4N[:,2,:]=1
        cam_points_b3N = K_b44[:, :3] @ spherical_points_b4N
        pix_coords_b2N = cam_points_b3N[:, :2]
        distance_b1N = distance_b1N.unsqueeze(1)
        return torch.cat([pix_coords_b2N, distance_b1N], dim=1)
