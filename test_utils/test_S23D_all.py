...
import modules.cost_volume_O as cost_volume_O
...
def main(opts):
    ...
    if (opts.fast_cost_volume and  
            isinstance(model.cost_volume, cost_volume_O.FeatureVolumeManager)):
    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            ...
            bounds = generate_bounds_for_scan_from_file(scan,'data_splits/S2D3D/S2D3D_camera_center.txt')
...
if __name__ == '__main__':
    ...
    sys.argv=['test_S2D3D.py', '--name','PANO_MODEL', '--output_base_path', './output/S2D3D/PanoGRF', '--config_file', 'configs/models/pano_model.yaml', '--load_weights_from_checkpoint', '/home/yzm/Workspace/simplerecon_v2/logs/PANO_MODEL/version_142/checkpoints/epoch=22-step=39191.ckpt', '--data_config', 'configs/data/S2D3D_default_test.yaml',
     '--num_workers', '2','--batch_size', '2','--run_fusion','--depth_fuser','ours','--fuse_color','--dump_depth_visualization']
    
    
    
    '''
    sys.argv=['test_OmniScenes_all.py', '--name','PANO_MODEL', '--output_base_path', './output/OmniScenes/Bifusev2', '--config_file', 'configs/models/pano_model.yaml', '--load_weights_from_checkpoint', '/home/yzm/Workspace/simplerecon_v2/logs/PANO_MODEL/version_142/checkpoints/epoch=22-step=39191.ckpt', '--data_config', 'configs/data/OmniScenes_default_test.yaml',
     '--num_workers', '2','--batch_size', '2','--run_fusion','--depth_fuser','ours','--fuse_color','--dump_depth_visualization']
    '''
    """
