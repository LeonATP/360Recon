...
def generate_bounds_for_scan_from_file(scan, file_path):
    """
    Find boundary values from the file by scan name and adjust boundaries 
    based on camera max and min values in each direction.
    
    Args:
        scan (str): Name of the scan.
        file_path (str): Path to the text file containing scene names and boundary values.
    
    Returns:
        dict: Boundary dictionary containing xmin, xmax, ymin, ymax, zmin, zmax.
    """
    try:
        ...
        for line in lines:
            ...
            scene_name = fields[0]
            # Find the scene matching the scan name
            if scene_name == scan:
                try:
                    xmax = float(fields[1])
                    xmin = float(fields[2])
        ...
        # 如果没有找到匹配的场景，返回默认值
        return None  # 默认返回 None 使用标准 bounds
...
def main(opts):
    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            bounds = generate_bounds_for_scan_from_file(scan,'data_splits/Matterport3d/matterport3d_camera_center.txt')  # 自定义函数生成扫描特定的 bounds
            ...
            for batch_ind, batch in enumerate(tqdm(dataloader)):
                ...
                valid_mask_b = valid_mask_b & (cur_data["full_res_depth_b1hw"] > 0.5)
                valid_mask_b = valid_mask_b & (upsampled_depth_pred_b1hw.squeeze(1) > 0)
...
if __name__ == '__main__':
    ...
    sys.argv=['test.py', '--name','PANO_MODEL', '--output_base_path', './output/matterport3d/Bifuse_v2', '--config_file', 'configs/models/pano_model.yaml', '--load_weights_from_checkpoint', '/home/yzm/Workspace/simplerecon_v2/logs/PANO_MODEL/version_122/checkpoints/epoch=17-step=133344.ckpt', '--data_config', 'configs/data/matterport_test.yaml',
     '--num_workers', '2','--batch_size', '2','--fuse_color','--dump_depth_visualization']
