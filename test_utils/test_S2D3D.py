...
def main(opts):
    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            bounds = generate_bounds_for_scan_from_file(scan,'data_splits/S2D3D/S2D3D_camera_center.txt')  # 自定义函数生成扫描特定的 bounds
...
if __name__ == '__main__':
    ...
    sys.argv=['test_S2D3D.py', '--name','PANO_MODEL', '--output_base_path', './output/matterport3d/S2D3D_wrap_feature', '--config_file', 'configs/models/pano_model.yaml', '--load_weights_from_checkpoint', '/home/yzm/Workspace/simplerecon_v2/logs/PANO_MODEL/version_146/checkpoints/epoch=25-step=44399.ckpt', '--data_config', 'configs/data/S2D3D_default_test.yaml',
