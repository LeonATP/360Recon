...
class S2D3DDataset(GenericMVSDataset):
    """ 
    ...
    def get_gt_mesh_path(dataset_path, split, scan_id):
        ...
        gt_path = os.path.join(
                        dataset_path,
                        S2D3DDataset.get_sub_folder_dir(split),
                        scan_id,
    ...
    def get_color_filepath(self, scan_id, frame_id):
        ...
        """
        scene_path = os.path.join(self.scenes_path,scan_id,"rgb")
        '''
    ...
    def get_cached_depth_filepath(self, scan_id, frame_id):
        ...
        """
        scene_path = os.path.join(self.scenes_path,scan_id,"depth")
        #sensor_data_dir = os.path.join(scene_path, "sensor_data")
    ...
    def get_pose_filepath(self, scan_id, frame_id):
        ...
        scene_path = os.path.join(self.scenes_path, scan_id,"pose")
        #sensor_data_dir = os.path.join(scene_path, "sensor_data")

        return os.path.join(scene_path, f"frame_{frame_id}.json")
    ...
    def load_full_res_depth_and_mask(self, scan_id, frame_id):
        ...
        full_res_depth = read_image_file(full_res_depth_filepath, 
                                                value_scale_factor=1e-3)
        '''
        #full_res_depth = self.load_hinter_depth(full_res_depth_filepath)
        
        full_res_depth = read_image_file(
                                full_res_depth_filepath,
                                height=self.native_depth_height,
                                width=self.native_depth_width,
                                value_scale_factor=1,
                                resampling_mode=pil.NEAREST,
                            )
        #full_res_depth = np.array(pil.open(full_res_depth_filepath),dtype=np.float32)
        full_res_depth=full_res_depth/512.0
        #full_res_depth = torch.from_numpy(full_res_depth)
        #full_res_depth = full_res_depth.unsqueeze(0)
    ...
    def load_pose(self, scan_id, frame_id):
        ...
        """
        self.load_capture_poses(scan_id,frame_id)


        pose_info = self.capture_poses
    ...
    def load_pred_depth(self, scan_id, frame_id):
        #scene_path = os.path.join("/home/yzm/Workspace/PanoFormer/PanoFormer/S2D3D/depth/", scan_id,'rgb')
        #scene_path = os.path.join("/home/yzm/Workspace/BiFusev2/S2D3D/depth/", scan_id,'rgb')
        #sensor_data_dir = os.path.join(scene_path, "sensor_data")
        frame_id=str(frame_id)
        #scene_path = os.path.join("/home/yzm/Workspace/fova-depth/test_logs/S2D3D_cube/lightning_logs/", scan_id,"visuals",frame_id)
        scene_path = os.path.join("/home/yzm/Workspace/PanoGRF/logs/mvsdepth/", scan_id)
        cached_resized_path = os.path.join(scene_path, 
                #f"frame_{frame_id}_depth.pkl")
                #f"frame_{frame_id}.pkl")
                #frame_id,
                #'original_depth.pkl')
                f"{frame_id}_depths.npz")
    ...
    def load_capture_poses(self, scan_id,frame_id):
        ...
        scene_path = os.path.join(self.dataset_path, 
                                scan_id,'pose',f"frame_{frame_id}.json")

        self.capture_poses= {}

        with open(scene_path, "r",encoding='UTF-8') as f:
            result=json.load(f)
            m = np.array(result["camera_rt_matrix"],dtype='float32')
            m=np.row_stack((m,[0,0,0,1]),dtype='float32')
